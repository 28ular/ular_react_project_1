import {useNavigate} from "react-router-dom";
import './shop.css'

export const Shop = ({ s , id }) => {

    const navigate = useNavigate();


    return (
        <>
       <div className="card">
           <div className="card_container">
               <div className="card_item">
                   <div className="title">
                       <div className="id">{s.id}</div>
                       {s.title}
                   </div>
                   <div className="image"><img src={s?.img || 'data:image/jpeg;base64,'} alt={s.description}/></div>
                   <div className="description">
                       {s?.description}
                   </div>
                   <div className="price">
                       {s?.price || '??'}
                   </div>
                   <button onClick={() => navigate(`/shop/${s.id}`)}>detail</button>
               </div>
           </div>
       </div>
        </>
    )
}